...
/**
*
 * Node - Defining the new Node Type
 *
*/
struct Node {
    Transaction currentTransaction;
    Node *previousNodeAddress = nullptr;
    string timestamp;
    ...
    Node(unsigned int value, const string &sender, const string &receiver);
};
...
/**
 * BlockTransactionValue - returns {+ or - or 0} specific for the name.
*/
long int BlockTransactionValue(const Node &node, const string &name);

/**
 * PrintDump - PRINTS SHIeT
*/
void PrintDump(Node *currentNode, ofstream &file, int number);

/**
 * DeleteRecursivelyNode - DELETES SHIeT
*/
void DeleteRecursivelyNode(Node *node);

/**
 * DeleteRecursivelyNode - DELETES all the SHIeT
*/
